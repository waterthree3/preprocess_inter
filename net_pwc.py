...
if __name__ == '__main__':
    #numpy.array(PIL.Image.open(args_strOne))[:, :, ::-1] RGB转BGR
    #transpose(2, 0, 1) HWC转CHW
    #astype(numpy.float32) * (1.0 / 255.0) 归一化
    ...
    tenTwo = tenTwo.unsqueeze(0).cuda()
    tenOutput = tenOutput.unsqueeze(0).cuda()
    warp_img = backwarp(tenTwo, tenOutput)
    #将BGR转换成RGB
    warp_img = warp_img[:, [2,1,0], :, :]
    #torchvision.utils.save_image(warp_img, args_strOut, nrow=1)
