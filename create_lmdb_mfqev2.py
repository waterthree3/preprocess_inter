...
def create_lmdb_for_mfqev2():
    ...
    #     frm_list.append([list(range(iter_seq * len_input + radius - 1, iter_seq \
    #         * len_input + radius + 2)) for iter_seq in range(num_seq)])
    # num_frm_total = sum([len(frms) * 3 for frms in frm_list])
    # msg = f'> {num_frm_total} frames found.'
    # print(msg)
    # key_list = []
    # video_path_list = []
    ...
    #             iter_seq+1, i) for i in range(3, 6)])
    #         video_path_list.extend([lq_video_list[iter_vid]] * 3)
